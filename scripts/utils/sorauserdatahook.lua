...
v4更新 
   MakeHook增加了优先级参数 用于控制hook调用顺序 数字越大优先调用
   推荐的基准优先级  实际使用时建议上下浮动一些 给其他mod留一些空间
   10000  我需要很高很高的优先级，请尽量第一个执行我的hook 一般用于修改参数之类的
   1000   我需要较高的优先级，请尽量前面执行我的hook 一般用于记录一些数据之类的
    0      默认优先级 如果不需要控制顺序 请直接用默认优先级 一般用于拦截一些调用
    -1000  我需要较低的优先级，请尽量后面执行我的hook 一般执行到这儿可以代表函数被允许执行了
   -10000 我需要很低很低的优先级，请尽量最后执行我的hook 
...
function UserDataHook.MakeHook(dataname,fnname,fn,priority)
    --hook哪个userdata的哪个函数 
    ...
    return {
        ...
        fnname   = fnname,
        fn = fn,
        priority = priority or 0,   --优先级 数字越大优先调用
...
function UserDataHook.Hook(inst,hook)
    ...
    table.sort(inst.userdatahooks[dataname][fnname],function(a,b) return (a.priority or 0) > (b.priority or 0) end) --排序 优先级高的在前面
    --切换成hook调用函数
    local fn 
