...
--[[
    ...
    主要方法：
    push    由com组件调用 用于推送事件到具体的世界 不推荐主动调用
    GetJson 由com组件调用 用于发送事件的组装json
    SysEvent    由com组件调用 用于处理事件接收和发送
    Clean       内部调用 用于清理缓存数据
]]--
...
--[[
    com类     一般挂在单例实体上 例如TheWorld 请保证同一namespace的组件只有一个在工作 否则结果未知
    用于Modder推送和接收事件
    主要方法:
...
function com:Nil()  end --使方法无效化


function com:SetNameSpace(namespace)  --设置命名空间 每个命名空间之间独立工作
    self.namespace = namespace or "worldevent"
    
end
function com:SetWorkMode(mode)  --设置工作模式 
...
function com:SetTimer(systime)  --设置同步时间 越短越耗性能 越长同步越慢
    self.systime  = systime or 10         --默认十秒同步一次 
end
function com:SysEvent()       --同步事件
...
function com:PushEvent(event,data,worldid,pushnow)       --推送事件到其他世界
...
function com:PushEventToWorld(event,data)       --读取事件并推送到自身事件 请勿调用
    self.inst:PushEvent(self.namespace..event,data)        --收到的事件会带上namespace  也可以自己直接外面监听
end
function com:ListenForEvent(event,fn)       --封装好的监听事件
    self.inst:ListenForEvent(self.namespace..event,fn)        --收到的时间会带上namespace  也可以自己直接外面监听
end
function com:RemoveEventCallback(event,fn)       --封装好的移除监听事件
