...
if light and light._lantern then light._lantern.sorabufferlight = nil 
end 
end
local onDead = nil 
...
local function cs(inst,target)
	if target and target:HasTag("Player") then
		...
		target.sorabuffercancer = function (inst) if inst.sorabufferlight ~= nil then inst.sorabufferlight:Remove() inst:RemoveEventCallback("onremove", onremove) end end
...
local function yy(inst,target)
	if target and target:HasTag("Player") then
		...
		target.sorabuffercancer = function (inst) if target.components.combat and target.components.combat.externaldamagemultipliers then
...
local function xs(inst,target)
	if target and target:HasTag("Player") then
		CancerBuffer(target)
		target.sorabuffertime= 60
		if target.components.combat  then
			target.components.combat.externaldamagemultipliers:SetModifier("sora2buffer",2)
			target.components.combat.externaldamagetakenmultipliers:SetModifier("sora2buffer",2)
		end
		target.sorabuffercancer = function (inst) if target.components.combat then
			target.components.combat.externaldamagemultipliers:SetModifier("sora2buffer")
		...
		target.sorabuffer = target:DoPeriodicTask(1,function (inst) 
					inst.sorabuffertime = inst.sorabuffertime -1
					if inst.components.health then
						inst.components.health:DoDelta(-5,nil,nil,nil,nil,true)
					end
					if inst.sorabuffertime < 1 then
					CancerBuffer(inst)
					end
...
local function ry(inst,target)
	if target and target:HasTag("Player") then
		...
		target.sorabuffercancer = function (inst) if inst.components.locomotor then
...
local function MakeBuffer(name,des,bufferfn)
	local assets = 
	{
	Asset("ANIM", "anim/sora2buffers.zip"),
...
return MakeBuffer("sora2qb","谦卑\r\n活力四射！",qb),
	MakeBuffer("sora2lh","灵魂\r\n精神焕发！",lh),
	MakeBuffer("sora2cs","诚实\r\n无惧黑暗！",cs),
	MakeBuffer("sora2lm","怜悯\r\n怜悯对手！",lm),
	MakeBuffer("sora2gz","公正\r\n远离寒暑！",gz),
	MakeBuffer("sora2yy","英勇\r\n英勇善战！",yy),
	MakeBuffer("sora2xs","牺牲\r\n向死而生！",xs),
	MakeBuffer("sora2ry","荣誉\r\n一往无前！",ry),
