...
require "debugtools"
local assets = {
    Asset("SOUND", "sound/sora.fsb"), -- Asset( "ANIM", "anim/sora.zip" ),
    Asset("ANIM", "anim/sora_dress.zip"),
    Asset("ANIM", "anim/sora_uniforms.zip"),Asset("ANIM", "anim/sora_sllh.zip"),
    ...
    Asset("ANIM", "anim/sorahair4.zip"),
    Asset("ANIM", "anim/ghost_sora_build.zip")
}
-- 追加新版本选人提示
if TUNING.GAMEMODE_STARTING_ITEMS then
    if TUNING.GAMEMODE_STARTING_ITEMS.DEFAULT then
        TUNING.GAMEMODE_STARTING_ITEMS.DEFAULT.SORA =
            {
                "cane_candycane", "cane_ancient", "cane_victorian",
                "cane_sharp", "sorabag", "sora2food"
            }
    end
    TUNING.STARTING_ITEM_IMAGE_OVERRIDE.sorabag =
        {atlas = "images/inventoryimages/sorabag.xml", image = "sorabag.tex"}
...
local testwlist = wlist({talk1 = 1, talk2 = 2, talk3 = 3})
...
for k, v in pairs(foods) do
    if v and v.name then table.insert(sorafoods, v.name) end
end
local function OnSoraSpawn(inst)
    ...
    if not first then inst.components.sorafl.has = true end
    if first and inst.components.inventory ~= nil and
        inst.components.inventory:GetNumSlots() > 0 then
        ...
        if gift then
            ...
            if ValidateRecipeSkinRequest(inst.userid, "cane", "cane_victorian") then
                table.insert(gifts, SpawnPrefab("cane", "cane_victorian", nil,
                                                inst.userid))
                cane = cane + 1
            end
            if ValidateRecipeSkinRequest(inst.userid, "cane", "cane_ancient") then
                table.insert(gifts, SpawnPrefab("cane", "cane_ancient", nil,
                                                inst.userid))
                cane = cane + 1
            end
            if ValidateRecipeSkinRequest(inst.userid, "cane", "cane_candycane") then
                table.insert(gifts, SpawnPrefab("cane", "cane_candycane", nil,
                                                inst.userid))
                cane = cane + 1
            end
            if ValidateRecipeSkinRequest(inst.userid, "cane", "cane_sharp") then
                table.insert(gifts, SpawnPrefab("cane", "cane_sharp", nil,
                                                inst.userid))
                cane = cane + 1
            end
            if ValidateRecipeSkinRequest(inst.userid, "cane", "cane_harlequin") then
                table.insert(gifts, SpawnPrefab("cane", "cane_harlequin", nil,
                                                inst.userid))
                cane = cane + 1
            end
            if cane < 2 then table.insert(gifts, SpawnPrefab("cane")) end
            if TheNet:GetServerGameMode() ~= "quagmire" then
                table.insert(gifts, SpawnPrefab("sora_shousi"))
                for i = 1, 3 do
                    table.insert(gifts, SpawnPrefab(
                                     sorafoods[math.random(1, #sorafoods)]))
                end
            end
...
local function applyupgrades(inst, nosay)
    inst.soralevel:set(exptolev(inst.soraexp:value()))
    inst.soraexpmax:set(expfornextlev(inst.soralevel:value()))
    ...
    for k, v in ipairs(SoraTags) do
        if not inst:HasTag(v) then inst:AddTag(v) end
    end
    if not inst:HasTag("reader") then inst:AddTag("reader") end
    if not inst.components.reader then inst:AddComponent("reader") end
end
        ...
        if (t - inst.soraday) > 0 then
            ...
            for k, v in pairs(olddayexp) do
                if k and v and v >= (maxexp * 0.75) then
                    inst.soradayexp[k] =
                        math.random(maxexp * 0.8, maxexp * 0.95)
                else
...
local deathlist = wlist({death1 = 1, death2 = 1, death3 = 1})
local function onbecameghost(inst)
    -- 死亡损失当前等级30%经验
    if inGamePlay then
        GetExp(inst, DeathExp(inst.soralevel:value()))
...
local function soracaneat(inst, food)
    if food ~= nil then
        if food.components.perishable and
            (food.components.perishable:IsStale() or
                food.components.perishable:IsSpoiled()) then return false end
...
local function onpick(inst, data)
    ...
    if not getsora("sbcj") then
        if inst.soralevel:value() > 14 and data.object and
            data.object.components.pickable and
            not data.object.components.trader then
            if data.object.plant_def and
                data.object.components.plantresearchable and
                data.object.components.pickable.use_lootdropper_for_product then
                local loot = {}
                for _, prefab in ipairs(
                                     data.object.components.lootdropper:GenerateLoot()) do
                    table.insert(loot,
                                 data.object.components.lootdropper:SpawnLootPrefab(
                                     prefab))
                end
                if not next(loot) and next(data.loot) then
                    for k,v in pairs(data.loot) do
                        if v and v.prefab then
                        table.insert(loot,data.object.components.lootdropper:SpawnLootPrefab(
                                     v.prefab))
                       end
                    end
                end
            ...
            elseif data.object.components.pickable.product ~= nil and
                ...
                local item =
                    SpawnPrefab(data.object.components.pickable.product)
...
local function onkill(inst, data)
    ...
    if inst.soralevel:value() > 29 and data.victim.components.lootdropper and
        not data.victim:HasTag("SoraKill") then
...
local function onattack(inst, data) GetExp(inst, 2, "attack", 50) end
...
local function onrepair(inst, data) GetExp(inst, 20, "repair") end
local function onsorarepair(inst, data) GetExp(inst, 20, "sorarepair") end
local function ondeployitem(inst, data) GetExp(inst, 3, "deployitem") end
...
local emotesoundlist = {
    ...
    carol = wlist({carol1 = 5, carol2 = 1, carol3 = 1, carol4 = 1, carol5 = 1}), -- carol
...
local function onnewstate(inst, data)
    ...
    if inst.sg.currentstate.name ~= "emote" then
        ...
        inst.components.sanity.dapperness =
            inst.components.sanity.dapperness - inst.soraindancing *
                soradancingsanity / 60
...
local common_postinit = function(inst)
    ...
    inst.soraexplocal = 0
    inst.soraexp = {
                        value = function() return inst.soraexplocal end,
                        set = function(a,b,v) inst.soraexplocal = b end,
                        setlocal = function() inst.soraexplocal = b end,
                    }
...
local master_postinit = function(inst)
    ...
    inst:ListenForEvent("healthdelta", function(inst, data)
        if data and data.afflicter and data.afflicter:IsValid() and
            ...
            local killer = data.afflicter.components.follower and
                               ...
                               data.afflicter or nil
            if killer and killer:HasTag("player") and killer ~= inst then
                killer.components.health:DoDelta(300 * data.amount, nil, nil,
                                                 true, killer, true) -- 300倍反伤
    ...
    inst.components.combat.overrideattackkeyfn = function() return "cryo" end
    ...
    inst.components.health.DoDelta = function(...)
        ...
        math.clamp = function(v, min, max, ...)
            if v > 1 and min == 0 and max == 1 then max = 999 end
    ...
    local moretag = {bramble_resistant = true}
    inst.components.inventory.EquipHasTag =
        function(self, tag) return moretag[tag] or EquipHasTag(self, tag) end

    inst.components.sanity.dapperness = -5 / 60
    inst.components.sanity:AddSanityAuraImmunity("tentacle")
    if GetModConfigData("foodmemory", TUNING.SORAMODNAME) and
        inst.components.eater then
        ...
        inst.components.eater.Eat = function(eater, food, feeder, ...)
            ...
            eater.hungerabsorption =
                eater.inst.components.sorafoodmemory:SoraGetFoodMultiplier(food)
    ...
    if inst.components.eater then
        -- fixeat
        local original_Eat = SoraUp.Get(inst.components.eater.Eat,
                                        "original_Eat")
...
local function MakeSkin(name,data,notemp)
    ...
    d.skins = {normal_skin = name, ghost_skin = "ghost_sora_build"}
...
MakeSkin( "sora_none", {
    ...
    des = '我是好孩子吧\n对吧?\n我要奖励！',
    quotes = '我的名字是"春日野穹"',
    skins = {normal_skin = "sora_uniforms", ghost_skin = "ghost_sora_build"},
...
return MakePlayerCharacter("sora", prefabs, assets, common_postinit,
