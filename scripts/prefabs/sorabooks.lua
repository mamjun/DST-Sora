...
local function trychange(inst)
    ...
    if  inst.components.perennialcrop2 and (
        inst.components.perennialcrop2.pollinated_max > -1  or 
        inst.components.perennialcrop2.infested_max < 100 or 
    ...
    if  inst.components.perennialcrop and not 
        inst.components.perennialcrop.sorafix then
...
local function TryToRemoveTentacle(doer,inst)
    ...
    for i=1,maxn,1 do
        if not(doer.tentacles and doer.tentacles[i] and doer.tentacles[i].components.health and not  doer.tentacles[i].components.health:IsDead() ) then
...

    {
        ...
        fn = function(inst, maker)
            ...
            if #ents > 100 then
                ...
                local san = AllRecipes and AllRecipes.sora_birds and AllRecipes.sora_birds.character_ingredients[1] and AllRecipes.sora_birds.character_ingredients[1].amount or 50
            ...
            reader:StartThread(function()
                for k = 1, numtentacles do
                    ...
                    if result_offset ~= nil then
                        ...
                        tentacle.components.combat:SetRange(TUNING.TENTACLE_ATTACK_DIST*1.5,TUNING.TENTACLE_ATTACK_DIST*2)
    ...
    local function fn()
		...
		inst.OnBuiltFn = function(inst,doer,...) 
            if doer and doer.components.debuffable and doer.components.debuffable:HasDebuff("myth_mooncake_icebuff") then
                ...
                local t = GetTime()
                local cd = AllRecipes and AllRecipes[def.name] and AllRecipes[def.name].character_ingredients[1] and AllRecipes[def.name].character_ingredients[1].amount or 50
                cd = cd / 10
            ...
            if doer and doer.components.builder and doer.components.builder.freebuildmode then
                local san = AllRecipes and AllRecipes[def.name] and AllRecipes[def.name].character_ingredients[1] and AllRecipes[def.name].character_ingredients[1].amount or 50
