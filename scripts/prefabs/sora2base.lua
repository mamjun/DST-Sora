...
]] local assets = {Asset("ANIM", "anim/sora2base.zip"), Asset("ANIM", "anim/sorachest.zip"),
...
local stoneassets = {Asset("ANIM", "anim/sora2stone.zip"), Asset("IMAGE", "images/inventoryimages/sora2stone.tex"),
                     Asset("ATLAS", "images/inventoryimages/sora2stone.xml"),
                     Asset("ATLAS_BUILD", "images/inventoryimages/sora2stone.xml", 256)}
local prefabs = {"moonrocknugget", "moonhound", "moonpig", "positronbeam_front", "positronbeam_back", "positronpulse",
                 "staffcoldlightfx"}
    ...
    firestaff = function(inst)
        ...
        if #ent > 0 then
            for i, v in ipairs(ent) do
                ...
                if sq < closest and
                    (v.prefab == "stafflight" or v.prefab == "staffcoldlight" or v.prefab == "staffcoldlightfx") then
                    closest = sq
                    closestinst = v
                end
            end
...
local function onclose(inst)
    ...
    inst.showlight = true
    if item2 and item2.prefab == "nightmarefuel" then
        inst.showlight = false
    end
...
local function fn()
    ...
    inst.components.inspectable:SetDescription([[插上不同的法杖可发光并获得不同效果
    冰魔杖->灭火      火魔杖->攻击
    传送魔杖->回城点  分解法杖->回血
    瞬移魔杖->回SAN
    唤星者->升温      唤月者->降温 
...
local function makestafflight(name)
    local assets = {Asset("ANIM", "anim/star_cold.zip")}
...
local function IsNearDanger(traveller)
    ...
    if traveller:HasTag("spiderwhisperer") then
        return FindEntity(traveller, 10, function(target)
            return (target.components.combat ~= nil and target.components.combat.target == traveller) or
...
local function onuse(inst, doer)
    ...
    if IsNearDanger(doer) then
        doer.components.talker:Say("太危险了")
        inst.components.sorauseable:StopUsingItem()
        return false
    end
...
local items = {{0.5, "small", "小"}, -- {0.7,"medium","中"},
{1, "big", "大"}}
SoraAPI.MakeItemSkinDefaultData("sora2base", {}, {})
...
return Prefab("sora2base", fn, assets, prefabs), Prefab("sora2stone", stonefn, stoneassets),
