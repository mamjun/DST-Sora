...
local seedinfo = Class(Widget, function(self,seeddata)
    Widget._ctor(self, "sora_seed_info")
    local btn = self:AddChild(ItemSlot(GetInventoryItemAtlas("tomato.tex"),"tomato.tex"))
...
local ui = Class(Widget, function(self,owner)
    ...
    for x = 1,5 do
        for y=1,5 do
            ...
            info:SetPosition(x*60-60,y*60-60,0)   
        end
    end
end)
