...
local env = {} -- 注册env环境
setmetatable(env, {__index = function(t, k) return rawget(soraupdate, k) or rawget(_G, k) end}) -- 注册全局表
local U 
...
local com = Class(function(self) -- 自动更新组件
    ...
    self:Redata()
    self.updatefn = scheduler:ExecutePeriodic(u.d and 5 or 60, function() self:TryToUpdate() end)
    self:TryToUpdate()
end)
...
function com:TryToUpdate()
    ...
    local j, postjson = pcall(json.encode, {cmd = "check", data = self.data})
    if postjson then
...
function com.filecallback(name, ishex,patch, version, file, result, code) -- 文件请求回调
    if code == 204 then print("PatchErrorNoFile",name,ishex,version) return end
    if code ~= 200 then if u.d then print("UpdateError",result) end return end
    if result then
        ...
        if success then
            local f = io.open("../mods/" .. TUNING.SORAMODNAME .. "/" .. name,
                              "w")
            if f then
                f:write(towrite)
                f:close()
