...
c.toprefabfn = function(item)
    if c.sales[item.prefab] then
        return item.prefab,
            c.sales[item.prefab] * (item.components.stackable and item.components.stackable.stacksize or 1)
    end
    ...
    if item.components.edible then
        ...
        if item.components.edible.healthvalue > 50 then
            p = p + 50
        end
        if item.components.edible.hungervalue > 50 then
            p = p + 50
        end
        if item.components.edible.sanityvalue > 50 then
            p = p + 50
        end
    end
...
local function AddSora(gifts, weight, level, exp, testfn, givefn)
    AddGift(gifts, weight, level, exp, function(s, doer, data)
        return doer and doer:HasTag("sora") and testfn(s, doer, data)
    end, givefn)
...
local function AddOzxz(t)
    for i = 1, 8 do
        AddGift(t, 0.05, i, 10, nil, ozxz)
    end
...
AddOzxz({
    ancienttree_gem_sapling_item = 5,
    opalpreciousgem = 5
})
