...
SORAPACK.fn = function(act)
    ...
    if invobject.prefab == "sora2pack" and target.prefab == "sorabowknot" then
        local num = invobject.components.stackable.stacksize *
                        exp[GLOBAL.TUNING.SORAMODE]
        ...
        if pack then
            if pack.components.sorapacker:Pack(target, doer) then
                ...
                if doer and doer.SoundEmitter then
                    doer.SoundEmitter:PlaySound(
                        "dontstarve/common/staff_dissassemble")
                end
                pack.components.inspectable:SetDescription(
                    pack.components.sorapacker.item.name)
...
AddAction(SORAPACK)
AddComponentAction("USEITEM", "sorapacker",
...
-- 打包盒拆包
local SORAUNPACK = GLOBAL.Action({rmb = true, priority = 2})
SORAUNPACK.id = "SORAUNPACK"
SORAUNPACK.str = "拆包"
SORAUNPACK.fn = function(act)
    local target = act.target or act.invobject
    local doer = act.doer
    if target ~= nil and target.components.sorapacker ~= nil and
        not target:HasTag("sorabowknot") then
...
AddStategraphActionHandler("wilson", GLOBAL.ActionHandler(
                               ACTIONS.SORAUNPACK, "doshortaction"))
AddStategraphActionHandler("wilson_client", GLOBAL.ActionHandler(
                               ACTIONS.SORAUNPACK, "doshortaction"))

-- 圣诞树领取礼物
local SORAGIFT = GLOBAL.Action({priority = 999})
...
AddStategraphActionHandler("wilson", GLOBAL.ActionHandler(
                               ACTIONS.SORAGIFT, "dolongaction"))
AddStategraphActionHandler("wilson_client", GLOBAL.ActionHandler(
                               ACTIONS.SORAGIFT, "dolongaction"))

-- 装备栏使用（勋章的切换）
local SORAUSE = GLOBAL.Action({priority = 99})
SORAUSE.id = "SORAUSE"
SORAUSE.str = "使用"
SORAUSE.fn = function(act)
    if act.invobject ~= nil and act.invobject.components.sorauseable ~= nil and
        act.invobject.components.sorauseable:CanInteract() and
...
AddAction(SORAUSE)
AddComponentAction("INVENTORY", "sorauseable",
                   function(inst, doer, actions, right)
    if not inst:HasTag("sorainuse") and doer.replica.inventory ~= nil and
...
-- 缝纫包修复
local SORAREPAIR = GLOBAL.Action({priority = 999})
SORAREPAIR.id = "SORAREPAIR"
SORAREPAIR.str = "修复"
SORAREPAIR.fn = function(act)
    if act.target ~= nil and act.doer ~= nil then
        if act.invobject.components.sorarepairer:DoRepair(act.invobject,
                                                          act.target, act.doer) then
...
AddComponentAction("USEITEM", "sorarepairer",
                   function(inst, doer, target, actions, right)
    if right and inst and target and target.replica.inventoryitem and
        target.replica.inventoryitem.classified and
...
AddStategraphActionHandler("wilson", GLOBAL.ActionHandler(
                               ACTIONS.SORAREPAIR, "dolongaction"))
AddStategraphActionHandler("wilson_client", GLOBAL.ActionHandler(
                               ACTIONS.SORAREPAIR, "dolongaction"))


local SORALINK = GLOBAL.Action({priority = 999})
...
AddStategraphActionHandler("wilson", GLOBAL.ActionHandler(
                               ACTIONS.SORALINK, "dolongaction"))
AddStategraphActionHandler("wilson_client", GLOBAL.ActionHandler(
                               ACTIONS.SORALINK, "dolongaction"))
                               
local SORABIND = GLOBAL.Action({priority = 999999})
...
local function UnBind(inst,doer,target,actions,...)
    if inst:HasTag("sorabind") then
        ...
        if inst and doer and not(doer.userid and inst:HasTag(doer.userid) or doer and doer.Network and doer.Network:IsServerAdmin()) then
            table.insert(act, ACTIONS.SORABIND)
        end
        if inst and doer and doer:HasTag("sora") and inst:HasTag("soranotlink") and doer.userid and inst:HasTag(doer.userid) then
