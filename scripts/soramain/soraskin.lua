...
MakeCharacterSkin("sora", "sora_llan", {
    ...
    rarityorder = 100,
    release_group = -1006,
    skins = {normal_skin = "sora_llan", ghost_skin = "ghost_sora_build"},
...
MakeCharacterSkin("sora", "sora_mysora", {
    ...
    rarityorder = 100,
    release_group = -1007,
    skins = {normal_skin = "sora_mysora", ghost_skin = "ghost_sora_build"},
    skin_tags = {"BASE", "sora", "CHARACTER"},
    checkfn = SoraSkinCheckFn,
    checkclientfn = SoraSkinCheckClientFn,
    share_bigportrait_name = "sora",
...
MakeCharacterSkin("sora", "sora_mysora_r", {
    ...
    rarityorder = 100,
    release_group = -1008,
    skins = {normal_skin = "sora_mysora_r", ghost_skin = "ghost_sora_build"},
    skin_tags = {"BASE", "sora", "CHARACTER"},
    checkfn = SoraSkinCheckFn,
    checkclientfn = SoraSkinCheckClientFn,
    share_bigportrait_name = "sora",
...
AddSimPostInit(function()
    ...
    GLOBAL.ShouldDisplayItemInCollection = function(str,...) 
        if (black[str] or str:match("^sora_.+_tmp$")) and not SoraSkinCheckFn(TheInventory,str)  then return false end
        return old(str,...) 
    end
end)
...
if not TheNet:IsDedicated() then
    ...
    AddSimPostInit(function(inst)
        ...
        TheWorld:DoPeriodicTask(300, function()
            ...
            if ThePlayer and ThePlayer:HasTag("sora") then OnLine() end
        end)
    end)
end
...
local skinhandle = {
    GetSkins = function(inst,Force)
        if not inst.soragetskincd then
            inst.soragetskincd = SoraCD(60)
            GetSkins(inst.userid)
        elseif inst.soragetskincd() or Force then
            GetSkins(inst.userid)
        end
    end
}
...
local function CheckBuild(inst, build)
    ...
    if not table.contains(PREFAB_SKINS.sora, build) then return false end
    if not inst:HasTag("sora") then return true end
...
if not TheNet:IsDedicated() then
    AddPrefabPostInit("sora", function(inst)
        inst:DoTaskInTime(5, function()
            ...
            if not ThePlayer.HUD then return end
            if selfowner['sora_dress'] then return end
            if token == '' then return end
            SkinApi('c/ActiveSkin', {skin = 'sora_dress'},
                    function(code, msg, data)
                if code == 5011 then
    ...
    -- AddPrefabPostInit("sora", function(inst)
        -- inst:DoTaskInTime(5, function()
            -- if inst ~= ThePlayer then return end
            -- if not ThePlayer.HUD then return end
            -- if selfowner['sora_gete'] then return end
            -- if token == '' then return end
            -- SkinApi('c/ActiveSkin', {skin = 'sora_gete_temp'},
                    -- function(code, msg, data)
                -- if code == 5031 then
                    -- SkinRPC('GetSkins')
                    -- GetSkins(selfid)
                    ...
                    -- scr.SetSkinName = function(s)
                        -- local skin_name =
                            -- string.lower(s.items[s.current_item].item)
                        -- s.item_name:SetColour(GetColorForItem(skin_name))
                        -- s.item_name:SetString(
                            -- GetSkinName(skin_name) .. '(限时)')
                        -- s.item_name:Show()
                        -- s.upper_banner_text:Show()
                    -- end
    ...
    -- AddPrefabPostInit("sora", function(inst)
        -- inst:DoTaskInTime(5, function()
            -- if inst ~= ThePlayer then return end
            -- if not ThePlayer.HUD then return end
            -- if selfowner['sora_zhizheng'] then return end
            -- if token == '' then return end
            -- SkinApi('c/ActiveSkin', {skin = 'sora_zhizheng_temp'},
                    -- function(code, msg, data)
                -- if code == 5041 then
                    -- SkinRPC('GetSkins')
                    -- GetSkins(selfid)
...
if not TheNet:IsDedicated() then
    ...
    local SkinActive = {
        sora_gete = function(s, item)  local scr = GameTimeUnLockScreen(item) 
    ...
    local item_map = {sora_none="sora_uniforms"}
            ...
            if gametimes > 300 then
                ...
                SkinApi('c/ActiveSkin', {skin = 'sora_gete'},
                    ...
                    elseif code == 5020 then
                        SoraPushPopupDialog('小穹的温馨提示',"你已经拥有了[穹妹-哥特萝莉],不需要重复激活")
                        SkinRPC('GetSkins')
                        GetSkins(selfid)
                ...
                SoraPushPopupDialog("小穹的温馨提示","在线时长不足50小时\n激活失败\n如果时长不对 请过半小时再试")
            end
        end
    end)
            ...
            if cdk then
                ...
                UseSkinCDK(cdk,function(code,msg,data)
                    ...
                    elseif (code == 6021 ) then
                        SoraPushPopupDialog('小穹的温馨提示',"你已经拥有了["..data.skinname.."],不需要重复激活")
                        SkinRPC('GetSkins')
                        GetSkins(selfid)
                    elseif (code == 6103 ) then
                    ...
                    elseif (code == 6001 ) then
                        ...
                        SoraPushPopupDialog('小穹的温馨提示',"皮肤激活失败,错误代码:"..code)
                    end
                end)
                return
            else
