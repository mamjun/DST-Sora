...
---------------------Server_Log-------------------------
%s
]],envinfo or "ERROR",playerinfo or "ERROR",retlog or "ERROR",ExtendTrace or "ERROR",extendinfo or "",serverinfo or "ERROR",serverlog or "ERROR")
        --给饥荒加一份 方便其他modder排查
        local last = 1
        local num = 0
    ...
    function getdebugstack(res, start, top, bottom,...)
        ...
        TheSim:QueryServer("http://jh.fl.lovetly.top/logupload.php?mod="..modkey.."&id="..logid,function() end,"POST",topost or "Error")--packlog(ret))
        --io.open()
        ...
        if err then table.insert(ret,1,err) end
