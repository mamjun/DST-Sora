...
local function makeconfig(name, label, des, default, min, step, num, ispercent)
    ...
    for i = 0, num, 1 do
        local da = min + i * step
        local desc = string.format(des,
                                   ispercent and (da - ispercent) * 100 or da)
        local index = i + 1
        t.options[index] = {
            description = desc,
        ...
        if tmp > -0.00001 and tmp < 0.00001 then
            finddefault = index
            t.options[index] = {
                description = desc .. "(默认)",
    ...
    if finddefault then
        local defaultoption = t.options[finddefault]
        for i = finddefault, 2, -1 do t.options[i] = t.options[i - 1] end
...
configuration_options = {
    {
        ...
        label = "难度",
        options = {
            {description = "一般", data = 2, hover = "人物较难升级"},
            {description = "简单", data = 1, hover = "人物容易升级"},
            {description = "困难", data = 3, hover = "人物很难升级"}
        },
        default = 2
    }, {
        name = "recmode",
        label = "配方难度",
        options = {
            {description = "默认", data = 0, hover = "跟随人物难度"},
            {
                description = "简单",
                data = 1,
                hover = "物品和装备容易制作"
        ...
        options = {
            ...
            {description = "兼容", data = 1, hover = "不进行更多尝试"}
        },
        default = 0
    }, {
        name = "foodmemory",
        ...
        label = "允许使用打包纸相关吗",
        options = {
            {description = "允许", data = true, hover = "允许打包"}, {
                description = "不允许",
                data = false,
        ...
        options = {
            {description = "允许", data = false, hover = "允许上强迫症"},
            {
                description = "禁止",
                data = true,
                hover = "不允许上强迫症"
        ...
        label = "修复飞行状态下距离判定",
        options = {
            {description = "修复", data = true, hover = "修复"},
            {description = "不修复", data = false, hover = "不修复"}
        },
        default = true
    }, {
        name = "lock2",
        ...
        label = "下线多少天后锁失效",
        options = {
            {description = "0", data = 0, hover = "永不失效"},
            {description = "1天", data = 1, hover = "下线1天后锁失效"},
            {
                description = "10天",
                data = 10,
                hover = "下线10天后锁失效"
        ...
        label = "扶光权限模式",
        options = {
            {description = "全局共享", data = 0, hover = "全局共享"},
            {description = "个人独立", data = 1, hover = "个人独立"}, {
        ...
        options = {
            {description = "3000", data = 3000, hover = "3000"},
            {description = "30", data = 30, hover = "30"},
            {description = "60", data = 60, hover = "60"},
            {description = "90", data = 90, hover = "90"},
            {description = "120", data = 120, hover = "120"},
            {description = "300", data = 300, hover = "300"},
            {description = "1000", data = 1000, hover = "1000"}
        },
        default = 3000
    }, maketitle("tongyong", "通用装备设置"),
    maketitle("sora3sword", "银白の锋"),
    makeconfig("att", "攻击力", "%s", 59.5, 17, 8.5, 10),
    makeconfig("spe", "移速", "%s%%", 1.1, 0.8, 0.05, 14, 1),
    makeconfig("att2", "攻击距离", "%s", 1.25, 0, 0.25, 12),
    makeconfig("att3", "回血", "%s", 0.5, -1, 0.5, 12),
    maketitle("sora2hat", "银白の盔"),
    makeconfig("arm1", "护甲", "%s%%", 0.85, 0.5, 0.025, 19, 0),
    makeconfig("arm2", "耐久", "%s", 2000, 500, 500, 9),
    makeconfig("wat", "防水", "%s%%", 0.3, 0, 0.05, 10, 0),
    makeconfig("spe", "移速", "%s%%", 1.1, 0.8, 0.05, 14, 1),
    makeconfig("san", "回SAN", "%s", 1, -5, 0.5, 20),
    maketitle("sora2armor", "银白の甲"),
    makeconfig("arm1", "护甲", "%s%%", 0.85, 0.5, 0.025, 19, 0),
    makeconfig("arm2", "耐久", "%s", 2000, 500, 500, 9),
    makeconfig("wat", "防水", "%s%%", 0.3, 0, 0.05, 10, 0),
    makeconfig("spe", "移速", "%s%%", 1.1, 0.8, 0.05, 14, 1),
    makeconfig("san", "回SAN", "%s", 1, -5, 0.5, 20),
    maketitle("sora2sword", "奇妙法杖"),
    makeconfig("spe", "移速", "%s%%", 1.1, 0.7, 0.05, 16, 1),
    makeconfig("att", "攻击", "%s", 10, 0, 5, 10),
    makeconfig("att2", "攻击距离", "%s", 8, 0, 1, 10),
    maketitle("sora2prop", "神奇法杖"),
    ...
    makeconfig("san", "回SAN", "%s", 1, -5, 0.5, 20),
    maketitle("sora2bag", "银白の翼"),
    makeconfig("spe", "移速加成", "%s%%", 1.1, 0.7, 0.05, 16, 1),
    makeconfig("san", "回SAN", "%s", 2, -5, 0.5, 20),
    maketitle("zhaunshu", "专属装备设置"),
    ...
    maketitle("sorapick", "穹の采"),
    makeconfig("att", "基础攻击力", "%s", 10, 0, 2, 20),
    ...
    makeconfig("foodd1", "基础反鲜(负的才是返鲜,正的是腐烂)",
               "%s", 0.25, -1, 0.05, 30),
    makeconfig("foodd2", "升级反鲜(负的才是返鲜,正的是腐烂)",
               "%s", -0.1, -0.5, 0.1, 20),
    makeconfig("spe", "移速", "%s%%", 1.1, 0.7, 0.05, 16, 1),
